...
def _inference_mode_collective_worker(
    rank: int,
    world_size: int,
    init_file: str,
    conn,
...
class TestDistributed(TestCase):
    ...
    @unittest.skipIf(os.name == "nt", "file init + spawn test is Linux-oriented")
    def test_inference_mode_allreduce_completes(self):
        world_size = 2
        fd, init_file = tempfile.mkstemp(prefix="mcpu_dist_inference_")
        os.close(fd)
        os.unlink(init_file)
        ctx = mp.get_context("fork")
        parent_conns = []
        ...
        for rank in range(world_size):
            ...
            procs.append(
                ctx.Process(
                    target=_inference_mode_collective_worker,
                    args=(rank, world_size, init_file, child_conn),
                )
            )
