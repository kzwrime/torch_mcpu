...
import torch
import torch.mcpu._gpu_trace as gpu_trace
...
# =============================================================================
# API Mappings: torch.xpu -> torch.mcpu/torch.accelerator
# =============================================================================
# Import aliases to keep the test code as close to the original as possible
import torch.mcpu
import torch.accelerator as _accelerator

# Basic module mapping
# torch.xpu = torch.mcpu
TEST_MULTIXPU = torch.mcpu.device_count() > 1
cpu_device = torch.device("cpu")
xpu_device = torch.device("mcpu")

# Stream and Event APIs - map to accelerator APIs
torch.xpu.Stream = torch.mcpu.Stream  # none
torch.xpu.current_stream = torch.accelerator.current_stream
torch.xpu.set_stream = torch.accelerator.set_stream
torch.xpu.stream = torch.mcpu.stream  # none
torch.xpu.Event = torch.Event
torch.xpu.synchronize = torch.accelerator.synchronize

# Memory APIs - map to accelerator APIs
torch.xpu.mem_get_info = torch.accelerator.get_memory_info
torch.xpu.empty_cache = torch.accelerator.empty_cache
torch.xpu.memory_allocated = torch.accelerator.memory_allocated
torch.xpu.memory_reserved = torch.accelerator.memory_reserved
torch.xpu.max_memory_allocated = torch.accelerator.max_memory_allocated
torch.xpu.max_memory_reserved = torch.accelerator.max_memory_reserved
torch.xpu.memory_stats = torch.accelerator.memory_stats
torch.xpu.reset_peak_memory_stats = torch.accelerator.reset_peak_memory_stats
torch.xpu.reset_accumulated_memory_stats = torch.accelerator.reset_accumulated_memory_stats

# Device context managers - map to accelerator APIs
torch.xpu.device = torch.accelerator.device_index
torch.xpu.current_device = torch.mcpu.current_device
torch.xpu.set_device = torch.mcpu.set_device
torch.xpu.device_count = torch.mcpu.device_count
# =============================================================================
...
@unittest.skipIf(not TEST_ACCELERATOR, "MCPU not available, skipping tests")
class TestXpu(TestCase):
    ...
    @unittest.skipIf(True, "not supported now")
...
def run_model(model, input):
    input_mcpu = input.clone().to('mcpu')
    model_mcpu = copy.deepcopy(model).to('mcpu')
    loss_mcpu = model_mcpu(input_mcpu).sum()
    loss = model(input).sum()
    torch.testing.assert_close(loss_mcpu.cpu(), loss)
...
"""
    ...
    def test_stream_event_repr(self):
        ...
        self.assertTrue("torch.Stream" in str(s))
        self.assertTrue("mcpu" in str(s))
        e = torch.xpu.Event()
        # self.assertTrue("torch.xpu.Event(uninitialized)" in str(e))
        s.record_event(e)
        self.assertTrue("torch.Event" in str(e))
    ...
    def test_generic_stream_event(self):
        ...
        event1 = torch.Event("mcpu", enable_timing=True)
        event2 = torch.Event("mcpu", enable_timing=True)
        self.assertEqual(event1.event_id, 0)
        a = torch.randn(1000)
        b = torch.randn(1000)
        with torch.xpu.stream(xpu_stream):
            a_xpu = a.to("mcpu", non_blocking=True)
            b_xpu = b.to("mcpu", non_blocking=True)
    ...
    def test_serialization_array_with_empty(self):
        x = [
            torch.randn(4, 4).mcpu(),
            torch.tensor([], dtype=torch.float, device=torch.device("mcpu")),
        ]
        with tempfile.NamedTemporaryFile() as f:
            torch.save(x, f)
            f.seek(0)
    ...
    def test_out_of_memory(self):
        ...
        tensor = torch.zeros(1024, device="mcpu")  # noqa: F841

        with self.assertRaisesRegex(RuntimeError, "Tried to allocate 800000000.00 GiB"):
            torch.empty(1024 * 1024 * 1024 * 800000000, dtype=torch.int8, device="mcpu")

        with self.assertRaisesRegex(RuntimeError, "XPU out of memory."):
            torch.empty(1024 * 1024 * 1024 * 8000000000, dtype=torch.int8, device="mcpu")
    ...
    def test_raises_oom(self):
        ...
        with self.assertRaises(torch.OutOfMemoryError):
            torch.empty(1024 * 1024 * 1024 * 1024, device="mcpu")
    ...
    def test_set_per_process_memory_fraction(self):
        ...
        tensor = torch.empty(application_memory, dtype=torch.int8, device="mcpu")
        del tensor
        gc.collect()
        torch.xpu.empty_cache()
        ...
        with self.assertRaises(torch.OutOfMemoryError):
            _ = torch.empty(application_memory, dtype=torch.int8, device="mcpu")
    ...
    def test_memory_stats(self):
        ...
        tmp = torch.randn(256, device="mcpu")
        # Detect if the current active memory is 1kB
        self.assertEqual(
    ...
    def test_mem_get_info(self):
        ...
        torch.randn(1024 * 256, device="mcpu")
        torch.xpu.synchronize()
...
"""
    ...
    def test_dlpack_conversion(self):
        ...
        x = make_tensor((5,), dtype=torch.float32, device="mcpu")
...
# instantiate_device_type_tests(TestXpuOps, globals(), only_for="mcpu", allow_xpu=True)
...
class TestXpuAutocast(TestAutocast):
    ...
    def setUp(self):
        super().setUp()
        self.autocast_lists = AutocastTestLists(torch.device("mcpu"))
        ...
        for op_with_args in self.autocast_lists.torch_fp16:
            ...
            if not skip_test:
                self._run_autocast_outofplace(
                    op, args, torch.float16, device="mcpu", amp_dtype=torch.float16
                )
    ...
    def test_autocast_torch_bf16(self):
        for op_with_args in self.autocast_lists.torch_fp16:
            ...
            if len(op_with_args) == 3:
                skip_test = True  # skip cudnn op
            if not skip_test:
                self._run_autocast_outofplace(op, args, torch.bfloat16, device="mcpu")

    def test_autocast_torch_need_autocast_promote(self):
        for op, args in self.autocast_lists.torch_need_autocast_promote:
            self._run_autocast_outofplace(
                op, args, torch.float32, device="mcpu", amp_dtype=torch.float16
            )
    ...
    def test_autocast_checkpointing(self):
        ...
        input = torch.rand(
            (8, 8), device="mcpu", dtype=torch.float16, requires_grad=True
        )
        for reentrant in (True, False):
            with torch.autocast("mcpu"):
    ...
    def test_xpu_autocast_dtype(self):
        dtype = torch.get_autocast_dtype("mcpu")
        self.assertEqual(dtype, torch.float16)
        mat0_fp32 = torch.randn((10, 10), dtype=torch.float32, device="mcpu")
        mat1_fp32 = torch.randn((10, 10), dtype=torch.float32, device="mcpu")
        with torch.amp.autocast("mcpu"):
...
class TestXPUAPISanity(TestCase):
    ...
    def test_torch_config_for_xpu(self):
        self.skipTest("Skipping test_torch_config_for_xpu.")
