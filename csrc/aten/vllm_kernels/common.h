...
// Check that a tensor has the expected floating-point dtype
// (float32/float16/bfloat16).
#define VLLM_MCPU_CHECK_FLOAT(t, name)                                     \
  VLLM_MCPU_CHECK(                                                         \
      (t).scalar_type() == at::kFloat || (t).scalar_type() == at::kHalf || \
          (t).scalar_type() == at::kBFloat16,                              \
      name " must be float32, float16, or bfloat16, got ",                 \
      (t).scalar_type())

// Check that a tensor has the expected scalar type.
// NOTE: parameter is named 'expected_st' to avoid shadowing tensor's .dtype()
// method.
#define VLLM_MCPU_CHECK_DTYPE(t, expected_st, name) \
  VLLM_MCPU_CHECK(                                  \
      (t).scalar_type() == (expected_st),           \
      name " must be " #expected_st ", got ",       \
      (t).scalar_type())

// Check tensor dimensions.
#define VLLM_MCPU_CHECK_DIM(t, d, name) \
  VLLM_MCPU_CHECK(                      \
      (t).dim() == (d), name " must be " #d "D, got ", (t).dim(), "D")
